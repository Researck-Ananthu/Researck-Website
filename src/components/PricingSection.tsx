...
const PricingSection = () => {
  return (
    <section id="pricing" className="w-full py-6 md:py-15 lg:py-17 xl:py-20 2xl:py-30">
      <div className="container selection-orange">
        {/* Heading */}
        ...
        <div className="mx-auto max-w-auto 2xl:max-w-8xl grid grid-cols-1 gap-6 px-4 sm:px-6 py-12 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {subscriptionPlans.map((plan, index) => (
            <Card
              key={plan.id}
              className={cn(
                "relative space-y-2 border-2 bg-muted/20",
                plan.isPopular ? "border-primary" : "",
                index === 0 && "border-dashed"
              )}
            >
              <CardHeader>
                <CardTitle className="text-xl font-bold break-words leading-tight sm:text-3xl">
                  {plan.name}
                </CardTitle>
                {plan.description && (
                ...
                {plan.monthlyPrice > 0 && (
                  <div>
                    ...
                    <div className="text-muted-foreground text-xs">
                      ${(plan.yearlyPrice / 100).toFixed(2)} /year (save ~15%)
                    </div>
                  </div>
                )}
              </CardHeader>
              ...
              <CardContent className="flex flex-1 flex-col gap-3">
                <p className="text-left text-xs md:text-sm font-medium">Features:</p>
